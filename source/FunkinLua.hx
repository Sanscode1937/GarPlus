...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('rating', 0);
		set('ratingName', '');
		set('version', MainMenuState.VERSION.trim());
		...
		Lua_helper.add_callback(lua, "startVideoDialogue", function(videoFile:String) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.modsVideo(videoFile))) {
				PlayState.instance.startVideoDialogue(videoFile);
			} else {
				luaTrace('Video and dialogue file not found: ' + videoFile);
			}
			#else
			if(PlayState.instance.endingSong) {
				PlayState.instance.endSong();
