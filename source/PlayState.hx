...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'garAlleyDead':
					...
					if (SONG.song.toLowerCase() == 'requiem')
						{
							var smokerr:FlxSprite = new FlxSprite(0, -290);
							smokerr.frames = Paths.getSparrowAtlas('garSmoke');
							smokerr.setGraphicSize(Std.int(smokerr.width * 1.7));
							smokerr.alpha = 0.3;
							smokerr.animation.addByPrefix('garsmoke', "smokey", 13);
							smokerr.animation.play('garsmoke');					
							smokerr.scrollFactor.set(0.7, 0.7);
							smokerr.color = FlxColor.fromRGB(69, 170, 207);
							add(smokerr);
						}	
			...
			case 'garAlleyDeadALT':
				...
				var bluesmoker:FlxSprite = new FlxSprite(0, -290);
				bluesmoker.frames = Paths.getSparrowAtlas('garSmoke');
				bluesmoker.setGraphicSize(Std.int(bluesmoker.width * 1.7));
				bluesmoker.alpha = 0.3;

				bluesmoker.animation.addByPrefix('garsmoke', "smokey", 13);
				bluesmoker.animation.play('garsmoke');					
				bluesmoker.scrollFactor.set(0.7, 0.7);
		...
		if (curStage == 'tank')
			...
			if (SONG.song.toLowerCase() == 'requiem' && SONG.song.toLowerCase() == 'addicted')
				{

					var smokerrr:FlxSprite = new FlxSprite(0, -290);
					smokerrr.frames = Paths.getSparrowAtlas('garSmoke');
					smokerrr.setGraphicSize(Std.int(smokerrr.width * 1.7));
					smokerrr.alpha = 0.3;
					smokerrr.animation.addByPrefix('garsmoke', "smokey", 13);
					smokerrr.animation.play('garsmoke');					
					smokerrr.scrollFactor.set(0.7, 0.7);
					smokerrr.color = FlxColor.fromRGB(69, 170, 207);
					add(smokerrr);
				}
		...
		if (curStage == 'garAlleyDeadALT') {

			var bluesmoker:FlxSprite = new FlxSprite(0, 0);
			bluesmokerr.frames = Paths.getSparrowAtlas('garSmoke');
			bluesmokerr.setGraphicSize(Std.int(bluesmokerr.width * 1.6));
			bluesmokerrequiem.alpha = 0.3;
			bluesmokerr.color = FlxColor.BLUE;
			bluesmokerr.animation.addByPrefix('garsmoke', "smokey", 15);
			bluesmokerr.animation.play('garsmoke');					
			bluesmokerr.scrollFactor.set(1.1, 1.1);
			add(bluesmokerr);
		}
		...
		timeTxt.setFormat(Paths.font("Milky Nice.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("Milky Nice.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'fading':
					trace("nooooooo he gone");
					garIntro(doof);	

				case 'reunion' | 'eased' | 'sweet-tooth' | 'sweet tooth' | 'stay':
				if(daSong == 'reunion') FlxG.sound.playMusic(Paths.music('city_ambience'));
				if(daSong == 'stay')  FlxG.sound.playMusic(Paths.music('city_ambience'));
				startDialogue(dialogueJson);
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Dodge Note': //Dodge Note
						...
						FlxG.sound.play(Paths.soundRandom('picoshoot-', 1, 3));					
